...
;	private OrderHelper oh =  OrderHelper.getHelper();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        /** 建立一個新的訂單物件 */
        Order od = new Order(username);

        /** 將每一筆訂單細項取得出來 */
        for(int i=0 ; i < item.length() ; i++) {
            String course_id = item.getString(i);
            
            // 學生註冊課程
            cuh.enroll(user_id, course_id);
            
            /** 透過 CourseHelper 物件之 getCourseById()，取得產品的資料並加進訂單物件裡 */
            Course c = ch.getCourseByID(course_id);
            od.addOrderItem(c);
        }
        ...
        /** 設定回傳回來的訂單編號與訂單細項編號 */
        od.setId(result.getInt("order_id"));
        od.setOrderItemId(result.getJSONArray("order_course_id"));
